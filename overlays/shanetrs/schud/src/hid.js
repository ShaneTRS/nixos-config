...
function eventListFromStateDiff(is, fs, p = "") {
	...
	for (let k in is) {
		...
		if (typeof is[k] === "object") events = events.concat(eventListFromStateDiff(is[k], fs[k], newpath));
		else if (is[k] !== fs[k]) events.push(newpath);
	}
	return events;
}
...
class SteamControllerDevice extends EventEmitter {
	constructor(steamControllerPath) {
		super();
		const matchingDevices = HID.devices().filter(
			(device) => device?.manufacturer?.match(/Valve/i) && device?.usagePage > 1,
		);
		...
		const userHid = document.getElementById("hid");
		this.steamControllerPath = steamControllerPath || matchingDevices[parseInt(userHid.value)].path;
		if (this.steamControllerPath) userHid.remove();
	...
	dispatch(event) {
		...
		let newevents = [];
		events.forEach((e) => {
			e.split("_").forEach((_, i, a) => newevents.push(a.slice(0, i + 1).join("_")));
		});
